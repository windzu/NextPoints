...
async def load_annotation(
    ...
    try:
        ...
        try:
            ...
            annotation_data = s3_service.read_json_object(project.bucket_name, annotation_key)
            if not annotation_data:
                return []
        except Exception as e:
            raise HTTPException(status_code=404, detail=f"Annotation not found: {annotation_key}, Error: {str(e)}")
        ...
        raise
        raise HTTPException(status_code=500, detail=f"Failed to load annotation: {str(e)}")
