...
from nextpoints_sdk.models.annotation import (
    AnnotationItem,
    PSR,
    Position,
    Rotation,
...
def create_mock_project_metadata():
    ...
    for i in range(3):
        ...
        frame = FrameMetadata(
            ...
            prev_timestamp_ns=(
                str(1500000000000000000 + (i - 1) * 100000000) if i > 0 else None
            ),
            next_timestamp_ns=(
                str(1500000000000000000 + (i + 1) * 100000000) if i < 2 else None
            ),
...
def create_mock_export_request():
    ...
    annotation_filter = AnnotationFilter(
        object_types=["Car", "Pedestrian"], min_points=10
    )

    return NuScenesExportRequest(
        coordinate_system=CoordinateSystem.GLOBAL, annotation_filter=annotation_filter
    )
...
def test_conversion():
    ...
    converter = NextPointsToNuScenesConverter(
        project_metadata=mock_project, export_request=mock_export_request
    )
    ...
    for file_path in output_dir.rglob("*"):
        if file_path.is_file():
            print(
                f"  {file_path.relative_to(output_dir)} ({file_path.stat().st_size} bytes)"
            )
                ...
                try:
                    ...
                    if "data" in sample and sample["data"]:
                        for sensor_channel, sample_data_token in sample["data"].items():
                            print(
                                f"Checking sensor {sensor_channel} with token {sample_data_token}"
                            )
                            try:
                                ...
                                print(
                                    f"Error getting sample_data for {sensor_channel}: {sd_error}"
                                )
                    else:
...
def test_category_mapping():
    ...
    for nextpoints_type, expected_nuscenes in test_cases:
        ...
        assert (
            result == expected_nuscenes
        ), f"Expected {expected_nuscenes}, got {result}"
...
def test_coordinate_transform():
    ...
    ego_pose = {"translation": [10.0, 20.0, 0.0], "rotation": [1.0, 0.0, 0.0, 0.0]}
    ...
    print(
        f"PSR transform: pos={global_position}, size={global_size}, rot={global_rotation}"
    )
