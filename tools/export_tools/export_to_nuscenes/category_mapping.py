...
def validate_mappings():
    # 提取类别和属性集合（假设里面都是 dict 且有 'name' 字段）
    cat_names = {c for c in NUSCENES_CATEGORIES}
    attr_names = {a for a in NUSCENES_ATTRIBUTES}

    # 1. CATEGORY_MAPPING 的 value 必须在类别集合内
    for v in CATEGORY_MAPPING.values():
        if v not in cat_names:
            raise ValueError(
                f"CATEGORY_MAPPING 的 value '{v}' 不在 NUSCENES_CATEGORIES 中"
            )

    # 2. DEFAULT_ATTRIBUTES 的 key 必须在类别集合内；value 必须在属性集合内
    for k, v in DEFAULT_ATTRIBUTES.items():
        if k not in cat_names:
            raise ValueError(
                f"DEFAULT_ATTRIBUTES 的 key '{k}' 不在 NUSCENES_CATEGORIES 中"
            )
        if v not in attr_names:
            raise ValueError(
                f"DEFAULT_ATTRIBUTES 的 value '{v}' 不在 NUSCENES_ATTRIBUTES 中"
            )

    # 3. CATEGORY_SIZE_CONSTRAINTS 的 key 必须在类别集合内
    for k in CATEGORY_SIZE_CONSTRAINTS.keys():
        if k not in cat_names:
            raise ValueError(
                f"CATEGORY_SIZE_CONSTRAINTS 的 key '{k}' 不在 NUSCENES_CATEGORIES 中"
            )
...
def get_nuscenes_category(nextpoints_type: str) -> str:
    ...
    if mapped not in NUSCENES_CATEGORIES:
        raise ValueError(
            f"Mapped category '{mapped}' not in NuScenes standard list for object type: {nextpoints_type}"
        )
    return mapped
