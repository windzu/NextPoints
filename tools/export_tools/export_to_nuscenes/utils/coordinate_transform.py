...
def transform_position_to_global(
    local_position: Position, ego_pose: Dict[str, Any]
) -> List[float]:
    """
    ...
    local = np.array(
        [local_position.x, local_position.y, local_position.z], dtype=float
    )
...
def transform_rotation_to_global(
    local_rotation: Rotation, ego_pose: Dict[str, Any]
) -> List[float]:
    """
...
def nuscenes_to_ego_pose_format(pose_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"translation": translation, "rotation": rotation, "timestamp": timestamp}
