...
function Annotation(sceneMeta, world, frameInfo) {
    ...
    // 将box从global(utm)坐标系变换到lidar坐标系
    // input : box在global(utm)坐标系下的标注结果
    // output : box在lidar坐标系下的位置
    ...
    */
    ...
    this.load_annotation = function (on_load) {
        if (this.data.cfg.disableLabels) {
            ...
            xhr.open('GET', "/api/legacy/load_annotation" + "?scene=" + this.frameInfo.scene + "&frame=" + this.frameInfo.frame, true);
            xhr.send();
        }
    };
