...
import { create_logger, logger } from "./log.js";
...
function Editor(editorUi, wrapperUi, editorCfg, data, name = "editor") {
    ...
    this.addRangeCircle = function () {
        ...
        for (var i = 0; i < segments; i++) {
            ...
            // body.push(0.6 * x1, 0.6 * y1, h, 0.6 * x2, 0.6 * y2, h); // 0.6*50=30m
            // body.push(1.0 * x1, 1.0 * y1, h, 1.0 * x2, 1.0 * y2, h); // 1.0*50=50m
            // body.push(2.0 * x1, 2.0 * y1, h, 2.0 * x2, 2.0 * y2, h); // 2.0*50=100m

            body.push(0.5 * x1, 0.5 * y1, h, 0.5 * x2, 0.5 * y2, h); // 0.5*50=25m
            body.push(0.8 * x1, 0.8 * y1, h, 0.8 * x2, 0.8 * y2, h); // 0.8*50=40m
            body.push(1.2 * x1, 1.2 * y1, h, 1.2 * x2, 1.2 * y2, h); // 1.2*50=60m
        }
    ...
    // 为gui添加表示地面的网格
    // Note : 实现的似乎不对 没有正确的显示
    this.addGround = function () {
        var size = 100; // 设置网格的大小
        var divisions = 10; // 设置网格的分割数

        // 创建一个网格助手对象
        var groundGeometry = new THREE.GridHelper(size, divisions);

        // 设置网格的位置
        groundGeometry.position.z = 0;

        // 将网格添加到场景中
        this.ground = groundGeometry;
        this.scene.add(this.ground);
    };

    // 显示或隐藏表示地面的网格
    this.showGround = function (show) {
        if (show) {
            if (this.data.world) {
                this.data.world.webglGroup.add(this.ground);
            }
        }
        else {
    ...
    this.transform_bbox = function (command) {
        ...
        switch (command) {
            ...
            case 'z_rotate_reverse': // rotate 180 degree
                ...
                break;
            case 'z_rotate_pi_2': // rotate 90 degree and not change the direction
                let tmp_scale_x = this.selected_box.scale.x;
                let tmp_scale_y = this.selected_box.scale.y;
                this.selected_box.rotation.z -= Math.PI / 2;
                this.selected_box.scale.x = tmp_scale_y;
                this.selected_box.scale.y = tmp_scale_x;

                if (this.selected_box.rotation.z < -Math.PI) {
                    this.selected_box.rotation.z += Math.PI * 2;
                }
                if (this.selected_box.rotation.z > Math.PI) {
                    this.selected_box.rotation.z -= Math.PI * 2;
    ...
    this.keydown = function (ev) {
        ...
        switch (ev.key) {
            ...
            */
            ...
            // 在框被选中的情况下，按下键盘上的w/a/s/d，可以上下左右移动框 q/e可以旋转框
            case 's':
                if (ev.ctrlKey) {
                    saveWorldList(this.data.worldList);
                }
            ...
            case 'g': // 反转选中框朝向180度
                ...
                break;
            case 'c': // 顺时针旋转选中框朝向90度
                this.transform_bbox("z_rotate_pi_2");
                break;
            case 't': // 显示轨迹
                this.showTrajectory();
                break;
