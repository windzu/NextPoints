...
import { dotproduct, euler_angle_to_rotate_matrix_3by3, matmul, transpose } from "./util.js";
...
function BoxOp() {
    ...
    // 判断自动调整结果是否合理
    // - 比较box的位置和原始box的位置的距离是否大于box的尺寸
    // - 比较box的尺寸和原始box的尺寸的比例是否大于3
    ...
    // - callback: 回调函数
    // - apply_mask: 应用mask
    // - on_box_changed: box改变时的回调函数
    // - noscaling: 是否启用缩放,true表示不启用缩放
    // - rotate_method: 旋转方法
    this.auto_rotate_xyz = async function (box, callback, apply_mask, on_box_changed, noscaling, rotate_method) {
        ...
        let grow = (box) => {
            ...
            // check if need to restore scale
            if (noscaling) {
        ...
        let postProc = (box) => {
            ...
            // if result is reasonable, we apply the result
    ...
    // 遍历box中的点,找到最大最小值,然后根据边界值对box进行收缩
    ...
    this.interpolateAndAutoAdjustAsync = async function (worldList, boxList, onFinishOneBoxCB, applyIndList, dontRotate) {
        // 获取所有box在global(utm)坐标系下的标注结果列表
        // 如果box为空或者box.annotator不为空,则返回null
        // if annotator is not null, it's annotated by us algorithms
        let anns = boxList.map((b, i) => {
            ...
            // 未被人工标注过而是由自动标注的框会有annotator 这个属性,值为a
            // 所以通过 annotator 判断是否是被人工标注过 没有则直接返回null
            if (b.annotator)
                return null;

            // 返回utm坐标系下的box的 size position rotation
            // input b : lidar坐标系下的box
            // output b : utm坐标系下的box(局部utm)
            return b.world.annotation.ann_to_vector_global(b);
        });


        // debug
        let annsCopy = JSON.parse(JSON.stringify(anns));
        // console.log("anns to interpolate", annsCopy);

        // 对输入的标注结果进行自动调整,自动调整项目包括
        // - 旋转自动调节(目前看来不是很好用)
        // - 对box的位置和尺寸进行调整(z值的调整有问题)
        // index: 当前标注的box的index
        // newAnn: 当前标注的box的新的标注结果
        // 返回值: 调整后的box的标注结果
        ...
        // 获取当前 instance 的 obj_type, obj_track_id, obj_attr
        let refObj = boxList.find(b => !!b);
        let obj_type = refObj.obj_type;
        let obj_track_id = refObj.obj_track_id;
        let obj_attr = refObj.obj_attr;
