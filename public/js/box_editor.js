...
function BoxEditorManager(parentUi, viewManager, objectTrackView,
    ...
    // Wire project management buttons
    const deleteBtn = this.toolbox.querySelector('#delete-project');
    if (deleteBtn) {
        deleteBtn.onclick = async () => {
            // delegate to the same handler bound in ConfigUi
            const menuDelete = document.querySelector('#cfg-delete-current-project');
            if (menuDelete) menuDelete.click();
        };
    }

    const createBtn = this.toolbox.querySelector('#create-project');
    if (createBtn) {
        createBtn.onclick = () => {
            const menuCreate = document.querySelector('#cfg-create-project');
            if (menuCreate) menuCreate.click();
        };
    }
